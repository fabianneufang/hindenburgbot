...
export type Autocomplete = (query: string | number, interaction: AutocompleteInteraction, document: GuildDocument) => Promise<Array<ApplicationCommandOptionChoice>>;
...
export type ContextMenuCommand = Command & {
  execute(interaction: ContextMenuInteraction, ephemeralPreference: boolean, target: string, document: GuildDocument): Promise<void>;
}

export type MentionCommand = Command & {
  execute(message: Message, reply: (options: string | MessageOptions) => Promise<Message>, args: Array<string>, document: GuildDocument, values: {content: string}): Promise<Message>;
